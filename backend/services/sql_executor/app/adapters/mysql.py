"""MySQL Database Adapter for Ask Your Data (Production-Ready)."""
...
class MySQLAdapter(BaseDatabaseAdapter):
    ...
    def _normalize_connection_string(self, connection_string: str) -> str:
        # If user passes mysql://, translate to mysql+pymysql:// for python driver
            ...
            for t_name in table_names:
                try:
                    ...
                    columns_intro = [
                        ColumnIntrospection(
                            name=c["name"],
                            data_type=str(c.get("type", "VARCHAR")),
    ...
    def execute_read_only(
        ...
        try:
            with engine.connect() as conn:
                # Set transaction read-only and statement execution timeout (in ms)
                try:
                ...
                try:
                    max_exec_ms = int(timeout_seconds * 1000)
                    conn.execute(text(f"SET SESSION max_execution_time = {max_exec_ms}"))
                except Exception:
                    pass
    ...
    def get_readonly_setup_instructions(self) -> str:
        return (
            ...
            "CREATE USER 'askyourdata_readonly'@'%' IDENTIFIED BY 'change_me_securely';\n"
            "-- 2. Grant SELECT privileges only on the target schema\n"
            "GRANT SELECT ON analytics_db.* TO 'askyourdata_readonly'@'%';\n"
            "-- 3. Flush privileges\n"
            "FLUSH PRIVILEGES;\n"
        )
