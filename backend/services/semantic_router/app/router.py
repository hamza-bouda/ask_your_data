...
Classifies user questions into specific intents using configured LLM Provider (DeepSeek / OpenAI)
and outputs a structured Semantic Plan.
"""
...
class SemanticPlanOut(BaseModel):
    ...
    source_tables: list[str] = Field(
        default_factory=list, description="List of table names selected from context."
    )
    metric: str | None = Field(None, description="The core business metric being requested.")
    dimensions: list[str] = Field(
        default_factory=list, description="Columns or attributes used to group or slice."
    )
    filters: list[str] = Field(
        default_factory=list, description="Specific conditions applied."
    )
    ...
    reasoning: str | None = Field(
        None, description="Brief explanation of why these tables and metrics were selected."
    )
...
def _is_catalog_request(query: str) -> bool:
    ...
    discovery_terms = (
        "liste", "list", "nom", "name", "quelle", "quelles", "which",
        "existe", "available", "disponible", "montre", "affiche", "show",
    )
    return any(term in normalized for term in catalog_terms) and any(
        term in normalized for term in discovery_terms
    )
...
def _mock_semantic_plan(query: str, context: dict) -> dict:
    """Deterministic, offline plan used only in development/mock test runtime."""
    normalized = query.lower()
    ...
    if normalized in {"montre-moi les données", "montre moi les données", "show me the data"}:
        return {
            ...
            "clarification_options": [
                "Quelle table et quelle mesure souhaitez-vous analyser ?",
                "Voulez-vous voir un aperçu de la table principale ?",
            ],
    ...
    if not tables:
        return {
            ...
            "confidence": 1.0,
            "clarification_options": [
                "Choisissez une source et une table autorisée.",
                "Demandez à l'administrateur d'ajouter des tables.",
            ],
...
def create_semantic_plan(query: str, context: dict, chat_history: list) -> dict:
    ...
    system_msg = (
        ...
        "If the user asks a follow-up question, use the 'payload' from the history to infer missing tables, metrics, or filters.\n"
        "\nIf the intent is DATA_QUERY or CHART_GENERATION, use the provided schema context (tables, relations, and CERTIFIED METRICS) "
        "to fill in 'source_tables', 'metric', 'dimensions', 'filters', 'period', 'grain' and 'reasoning'. "
        "CRITICAL: If the user requests a certified metric, use it strictly. If multiple match, classify as AMBIGUOUS. "
        "Populate 'business_provenance' with metadata about the chosen metric."
    )
    ...
    try:
        ...
        structured_llm = provider.with_structured_output(SemanticPlanOut)
        ...
        print(f"Error creating semantic plan with LLM: {e}")
        return {
            "intent": "AMBIGUOUS",
            "confidence": 0.0,
            "clarification_options": ["Réessayez votre question dans quelques instants."],
            "reasoning": f"Semantic routing is temporarily unavailable: {str(e)}",
        }
