...
@pytest.mark.asyncio
async def test_sse_flow_and_run_creation(override_get_db):
    mock_redis = AsyncMockRedis()
    with patch("backend.services.orchestrator.app.main.get_redis_client", return_value=mock_redis), \
         patch("backend.services.orchestrator.app.redis_client.get_redis_client", return_value=mock_redis):
